...
! tmelt_util           temperature of melting point for water         K
...
real(r8) :: xxlv_util        ! vaporization
real(r8) :: xxls_util        ! sublimation
...
! "kind" serv_utiles no purpose here except to check for unlikely linking
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:tmp,pgamp1,shapec,lbnd,ubnd)
#endif // defined(OPENMP_GPU)
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:tmpa)
#endif // defined(OPENMP_GPU)
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:ab,qiic,niic,lami,n0i)
#endif // defined(OPENMP_GPU)
  ...
  call calc_ab(t, qvi, xxls_util, ab, vlen)
...
!$omp loop bind(teams)
  ...
  do i=1,vlen
     if (qi(i)>=qsmall) then
        ...
        if (t(i) < tmelt_util .and. vap_dep(i) > 0._r8) then
           ice_sublim(i) = 0._r8
        else
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:prc_coef)
#endif // defined(OPENMP_GPU)
...
#endif // defined(OPENMP_GPU)
  do i=1,vlen
     if (t(i) <= tmelt_util .and. qiic(i) >= qsmall) then
        d_rat = lami(i)*dcs
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:nslip,ndfaer)
#endif // defined(OPENMP_GPU)
...
#endif // defined(OPENMP_GPU)
  do i=1,vlen
     if (qsic(i) >= qsmall .and. t(i) <= tmelt_util .and. qcic(i) >= qsmall) then
...
#endif // defined(OPENMP_GPU)
  do i=1,vlen
     if (qric(i) >= icsmall .and. qsic(i) >= icsmall .and. t(i) <= tmelt_util) then
  ...
  do i=1,vlen
     if (t(i) < 269.15_r8 .and. qric(i) >= qsmall) then
        nnuccr(i) = pi*nric(i)*bimm* &
             (exp(aimm*(tmelt_util - t(i)))-1._r8)/lamr(i)**3
...
#if defined(OPENMP_GPU)
!$omp target data map(alloc:pra_coef)
#endif // defined(OPENMP_GPU)
     ...
     if (precip_frac(i) > dum(i)) then
        ...
        if (qric(i) >= qsmall) then
           if (.not.evap_rhthrsh.or.(evap_rhthrsh.and.qclr/qvl(i).le.0.9_r8)) then
              call calc_ab(t(i), qvl(i), xxlv_util, abr)
        ...
        if (qsic(i) >= qsmall) then
           call calc_ab(t(i), qvi(i), xxls_util, ab)
  ...
  do i=1,vlen
     ...
     if (precip_frac(i) > dum(i)) then
        ...
        if (qgic(i).ge.qsmall) then
           call calc_ab(t(i), qvi(i), xxls_util, abg)
...
#endif // defined(OPENMP_GPU)
  do i=1,vlen
     if (qsic(i) >= qsmall.and. qcic(i) >= qsmall .and. t(i) < tmelt_util) then
        call calc_ab(t(i), qvi(i), xxls_util, ab)
        eps = 2._r8*pi*n0s(i)*rho(i)*Dv(i)* &
             (f1s/(lams(i)*lams(i))+ &
             f2s*(asn(i)*rho(i)/mu(i))**0.5_r8* &
...
!Output: NMULTG,QMULTG,NMULTRG,QMULTRG,PSACWG,PRACG
  ...
  tm_3 = tmelt_util - 3._r8
  tm_5 = tmelt_util - 5._r8
  tm_8 = tmelt_util - 8._r8
...
!  do i=1,vlen
!     abi = calc_ab(t(i), qvi(i), xxls_util)
!      if (qgic(i).ge.qsmall) then
...
SUBROUTINE kr_externs_in_micro_mg_utils(kgen_unit) 
    ...
    READ (UNIT = kgen_unit) rv_util 
    READ (UNIT = kgen_unit) cpp_util 
    READ (UNIT = kgen_unit) tmelt_util 
    READ (UNIT = kgen_unit) ra 
    READ (UNIT = kgen_unit) xxlv_util 
    READ (UNIT = kgen_unit) xxls_util 
