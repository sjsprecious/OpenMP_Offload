...
real(r8) :: tmelt_wv   ! Melting point of water at 1 atm (K)
...
!$acc declare copyin (epsilo, tmelt_wv, tboil, omeps, h2otrip)
#if defined(OPENMP_GPU)
!$omp declare target (epsilo,tmelt_wv,tboil,omeps,h2otrip)
...
#endif // defined(OPENMP_GPU)
  do i = 1, vlen
     es(i) = c1*exp( (c2*(t(i) - tmelt_wv))/((t(i) - tmelt_wv)+c3) )
  end do
  !$acc end parallel
#if defined(OPENMP_GPU)
!$omp end target teams
...
#if defined(OPENMP_GPU)
!$omp target update to (tmelt_wv,h2otrip,tboil,epsilo,omeps)
#endif // defined(OPENMP_GPU)
